...
public class TemplateController {
    ...
    @PostMapping(value = "/findTemplateformap/{categoryId}" )
    public Result<Template> findTemeplateformap(@PathVariable(value = "categoryId") Integer categoryId){
        //调用TemplateService实现分页条件查询Template
        Template template = (Template) brandService.all(categoryId).get("template");

        return new Result(true, StatusCode.OK,"查询成功",template);
    }
    //  模板加载
    @PostMapping(value = "/findTemplate/{categoryId}" )
    public Result<Template> findTemeplate(@PathVariable(value = "categoryId") Integer categoryId){
        //调用TemplateService实现分页条件查询Template
        Template tempalte = templateService.findTempalte(categoryId);
        return new Result(true, StatusCode.OK,"查询成功",tempalte);
    }
    /***
     * Template分页条件搜索实现
     * @param template
