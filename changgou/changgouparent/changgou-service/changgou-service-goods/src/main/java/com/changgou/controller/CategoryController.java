...
public class CategoryController {
    ...
    @RequestMapping("/findByCategory1ParentId/{parentId}")

    public  Result<List<Category>> findCategory1ByParentId(@PathVariable(value = "parentId") Integer id){
        List<Category> list = categoryService.findByCategory1ParentId(id);
        return new Result<>(true,StatusCode.ACCESSERROR,"查询成功",list);

    }
    @RequestMapping("/findByCategory2ParentId/{parentId}")

    public  Result<List<Category>> findCategory2ByParentId(@PathVariable(value = "parentId") Integer id){
        List<Category> list = categoryService.findCategory2ByParentId(id);
        return new Result<>(true,StatusCode.ACCESSERROR,"查询成功",list);

    }
    @RequestMapping("/findByCategory3ParentId/{parentId}")

    public  Result<List<Category>> findCategory3ByParentId(@PathVariable(value = "parentId") Integer id){
        List<Category> list = categoryService.findCategory3ByParentId(id);
        return new Result<>(true,StatusCode.ACCESSERROR,"查询成功",list);
